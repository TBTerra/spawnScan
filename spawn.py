...
def worker(wid,Tthreads):
	print 'worker {} is doing steps {} to {}'.format(wid,int((wid*len(scans))/Tthreads),int(((wid+1)*len(scans))/Tthreads))
	#login
	api = PGoApi()
	...
	for i in xrange(int((wid*len(scans))/Tthreads),int(((wid+1)*len(scans))/Tthreads)):
	...
	for i in xrange(int((wid/Tthreads)*len(scans)),int(((wid+1)/Tthreads)*len(scans))):
		doScan(scans[i][0], scans[i][1], api)
	curTime=time.time()
	print 'worker {} took {} secconds to do second pass now sleeping for {}'.format(wid,curTime-startTime,600-(curTime-startTime))
	time.sleep(600-(curTime-startTime))
	print 'worker {} is doing third pass'.format(wid)
	for i in xrange(int((wid/Tthreads)*len(scans)),int(((wid+1)/Tthreads)*len(scans))):
		doScan(scans[i][0], scans[i][1], api)
	curTime=time.time()
	print 'worker {} took {} secconds to do third  pass now sleeping for {}'.format(wid,curTime-startTime,600-(curTime-startTime))
	time.sleep(600-(curTime-startTime))
	print 'worker {} is doing forth pass'.format(wid)
	for i in xrange(int((wid/Tthreads)*len(scans)),int(((wid+1)/Tthreads)*len(scans))):
		doScan(scans[i][0], scans[i][1], api)
	curTime=time.time()
	print 'worker {} took {} secconds to do fourth pass now sleeping for {}'.format(wid,curTime-startTime,600-(curTime-startTime))
	time.sleep(600-(curTime-startTime))
	print 'worker {} is doing fith pass'.format(wid)
	for i in xrange(int((wid/Tthreads)*len(scans)),int(((wid+1)/Tthreads)*len(scans))):
		doScan(scans[i][0], scans[i][1], api)
	curTime=time.time()
	print 'worker {} took {} secconds to do fith pass now sleeping for {}'.format(wid,curTime-startTime,600-(curTime-startTime))
	time.sleep(600-(curTime-startTime))
